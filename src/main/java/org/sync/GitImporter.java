...
public class GitImporter {
			...
			try {
				...
				if(current.isFileDelete()) {
					...
					if(null != revision) {
						...
						repositoryHelper.updateFileVersion(head, path, f.getViewVersion(), f.getContentVersion(), f.getMD5());
				...
				if(null != lastCommit && lastInformation.equivalent(current)) {// 判断两次相邻的commitinformation是否可以归为一次提交
					...
					if(null == lastCommit) {
						...
						commit.setFromCommit(lastCommit);// 设置上一个 commit，形成commit链表
					}
					if(fattributes != null) {
						fattributes = null;
					}
	...
	public void generateByLabelImport(View view, Date date, String baseFolder, String labelFilteringPattern, String changeRequestFilePattern) {
		...
		if (viewLabels.length > fromLabel)
		{
			firstTime = viewLabels[viewLabels.length-1].getTime().getLongValue();
		}
		else if (isResume)
		{
		...
		while (timeIncrement.getTimeInMillis() <= lastTime) {
			...
			if (lastTime - timeIncrement.getTimeInMillis() > 7*day) {
				...
				timeIncrement.add(Calendar.DAY_OF_YEAR, 1);
			}
		}
	}
	/**
	...
	public void generateAllLabelImport(View view, String baseFolder) {
		...
		/* 最近的label 到当前状态生成 commit*/
		if (viewLabels != null && viewLabels.length > 0) {
		    firstTime = viewLabels[viewLabels.length-1].getTime().getLongValue();
        }
		...
		while (timeIncrement.getTimeInMillis() <= lastTime) {
			View vc;
			if(lastTime - timeIncrement.getTimeInMillis() <= day) {
				vc = view;// 最近的修改距离上一次提交小于一天，则生成这段时间内的commits
			} else {
			    // 最近的修改距离上一次提交大于一天，则生成 timeIncrement 到上一次提交这段时间的commits
				vc = new View(view, ViewConfiguration.createFromTime(new OLEDate(timeIncrement.getTimeInMillis())));
			}
			Log.log("View Configuration Time: " + timeIncrement.getTime());
			generateFastImportStream(vc, baseFolder);
	...
	private Set<String> skipView = new HashSet<>();
	/**
	 * 递归找到所有的视图，包括：
	 * 根视图
	 * 根视图衍生出的视图（可以找到衍生点，某个label）
	 * @param project
	 * @param rootView
	 * @param skipPatternStr
	 * @return
	 */
	private List<View> getAllViews(Project project, View rootView, String skipPatternStr) {
		...
		while(!deque.isEmpty()) {
			...
			try {
				...
				if (tag == null && view != rootView) {// 不是根视图，也没有找到重合的label，说明不是该根视图的衍生视图，不能构成分支关系
	...
	private Map<String, String> viewRef = new HashMap<>();
	public void generateAllViewsImport(Project project, View rootView, String baseFolder, String skipPattern) {
		...
		for (View view: views) {
			...
			java.util.Date startDate = new java.util.Date(0); // initialize with most
			                                                  // initial date
			if(baseRef != null) {// baseRef != null 说明该视图是从父视图衍生出的
...
//		return refName(label.getName());
	...
	private String getBaseTag(View v) {
		ViewConfiguration vc = v.getBaseConfiguration();// Returns the ViewConfiguration from which this View was derived.
		if (vc.isLabelBased()) {
	...
	public Set<String> getSkipViews() {
	    return skipView;
	}
	 ...
	 * git reset --hard xxx
	 */
	public void resetHEAD(String branchName) {
	    String workingDirectory = repositoryHelper.getWorkingDirectory();
	    boolean success = false;
	    Git git = null;
	    Repository repo = null;
	    try {
            repo = new FileRepositoryBuilder().setGitDir(new java.io.File(workingDirectory)).build();
            git = new Git(repo);
            List<Ref> branchs = git.branchList().call();
            for (Ref ref : branchs) {// 获取所有分支
                String fullRef = ref.getName();
                String prefix = "refs/heads/";
                String br = fullRef.substring(prefix.length());
                if (br.equals(branchName)) {
                    git.reset().setMode(ResetType.HARD).setRef(fullRef).call();
                    success = true;
                    break;
                }
            }
            if (!success) {
                Log.log("reset --hard " + branchName + " failed! " + " Can not find " + branchName + " !");
            }
        } catch (Exception e) {
            Log.log("reset --hard " + branchName + " failed! " + e.getMessage());
        } finally {
            if (git != null) {
                git.close();
            }
	...
	public String getWorkingDirectory() {
	    return repositoryHelper.getWorkingDirectory();
	}
}
