...
public class MainEntry {
	...
	public static void main(String[] args) {
		...
		CmdLineParser.Option excludeView = parser.addStringOption("exclude-view");//添加参数，去除子视图，与-A参数一同使用有效
...
//		boolean allViews = allViewsFlag != null && allViewsFlag;
		...
		if (pathToProgram == null) {
            pathToProgram = Config.instance.get("gitexe");
        }
//		Boolean createNewRepo = (Boolean) parser.getOptionValue(isCreateRepo);
		Boolean createNewRepo = true;
		...
		Vector<String> excludedLabels = parser.getOptionValues(excludeLabel);
...
//		if(host == null || port == null || project == null || (view == null && !allViews)) {
	...
	private static void importEnd(Map<String, String> importedViews, Set<String> skipViews) {
        ...
        for (Map.Entry<String, String> e : importedViews.entrySet()) {
            Log.log("Imported view " + e.getKey() + " onto " + e.getValue());
        }
        Log.log(">>>>>>>>>>>>>>>>>>>>>>>Skiped View>>>>>>>>>>>>>>>>>>>>>>>>");
        for (Iterator iterator = skipViews.iterator(); iterator.hasNext();) {
           Log.log((String) iterator.next());
        }
        Log.log(">>>>>>>>>>>>>>>>>>>>>>>Import End>>>>>>>>>>>>>>>>>>>>>>>>>");
	}
